...
date_min <- reactive( all_data() %...>% pull(date) %...>% min() )
date_max <- reactive( all_data() %...>% pull(date) %...>% max() )
...
compute_count_by_date <- function(n = 25) {
  packages_summary_by_date() %...>% {
    indices <- seq(1, nrow(.), length.out = n)
    .[indices,]
  }
}
...
# Dependencies data
all_deps <- reactive({ download_data() %...>% extract2("deps") })
...
server <- function(input, output) {
  ...
  packages_released_on_date <- reactive({
    ...
    all_data() %...>% {
      released_packages <- {.} %>% filter(date == input$date) %>% pull(Package)
  ...
  output$date_slider_ui <- renderUI({
    promise_all(date_min = date_min(), date_max = date_max()) %...>% {
  ...
  output$package_version_selector <- renderUI({
    selected_package_data() %...>%
      pull(Version) %...>%
      selectInput("package_version", "Version", ., selectize = FALSE, width = "150px")
  })
  ...
  output$package_timeline <- renderImage({
    selected_package_data() %...>%
      plot_cache(input$package, .) %...>%
      list(src = ., width = "100%", height = "auto")
  }, deleteFile = FALSE)
